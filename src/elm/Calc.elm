...
stepEpoch ({ jackByID } as model) calc =
    let
        jacks =
            Dict.values jackByID

        inputs =
            List.filter (\j -> j.direction == Input) jacks

        outputs =
            List.filter (\j -> j.direction == Output) jacks
    in
        calc
            |> updateEpoch
            |> updateFlows inputs
...
getInitialCalc model =
    let
        jackByID =
            model.jackByID |> Dict.map (\i j -> { flow = 0 })

        containerByID =
            model.containerByID |> Dict.map (\_ { initialAmount } -> { amount = initialAmount })
...
getCalc model =
    let
        ...
        step =
            purememo .epoch (stepEpoch model)

        ( calc, cache ) =
            Purememo.repeat step model.calcCache model.epoch initial
    in
        ( calc, cache )
