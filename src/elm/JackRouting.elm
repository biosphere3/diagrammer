...
linkPath model process container =
    let
        processPosition =
            getProcessPosition model process

        containerPosition =
            getContainerPosition model container

        processShape =
            Shape.getProcessShape process

        containerShape =
            Shape.getContainerShape container
...
inflectionPoints plan =
    let
        ( ( px, py ), ( cx, cy ) ) =
            tupmap2 toTuple plan.contacts

        axes =
            tupmap2 axis plan.faces
...
orthoPlan (( shape1, position1 ) as thing1) (( shape2, position2 ) as thing2) =
    let
        ...
        getPlan ( f1, f2 ) =
            let
                p1 =
                    (contactPoint thing1 f1)

                p2 =
                    (contactPoint thing2 f2)
...
possibleFacePairs p1 p2 =
    let
        ( f1a, f2a ) =
            if getX p2 > getX p1 then
                ( East, West )
            else
                ( West, East )

        ( f1b, f2b ) =
            if getY p2 > getY p1 then
                ( South, North )
            else
                ( North, South )
    in
